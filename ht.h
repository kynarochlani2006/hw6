...
public:
    ...
    HASH_INDEX_T next() 
    {
        ...
        HASH_INDEX_T begin = this->start_;
        HASH_INDEX_T step = dhstep_;
        size_t i = this->numProbes_;

        HASH_INDEX_T returnVal = (begin + i * step) % this->m_;
        this->numProbes_++;
        return returnVal;
...
{
  size_t currSize = numoccupied;
  HashItem* doesExist = internalFind(p.first);
  bool isNew = false;
  if(doesExist == nullptr){
    isNew = true;
  }

  if(isNew && (static_cast<double>(numoccupied) / CAPACITIES[mIndex_]) >= resizeAlpha_){
    resize();
  }
...
void HashTable<K,V,Prober,Hash,KEqual>::resize()
{
  ...
  if(mIndex_ + 1 >= sizeof(CAPACITIES)/ sizeof(HASH_INDEX_T)){
      throw std::logic_error("No resize");
  }
  mIndex_++;
  ...
  for(size_t i = 0; i < oldTable.size(); i++){
    if(oldTable[i] != nullptr && oldTable[i]->deleted == false){
        HASH_INDEX_T hasher = hash_(oldTable[i]->item.first) % newSize;
        prober_.init(hasher, newSize, oldTable[i]->item.first);
        HASH_INDEX_T pos = prober_.next();
        while(pos != npos && table_[pos] != nullptr){
          pos = prober_.next();
        }
        if(pos == npos){
            throw std::logic_error("Fail");
        }
      table_[pos] = new HashItem(oldTable[i]->item);
      numoccupied++;
    }
    delete oldTable[i];
  }
